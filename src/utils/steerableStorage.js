export default class WebStorage {
  ...
  static formatName(name, namespace) {
      ...
      if (namespace.trim().length === 0) { throw new TypeError('namespace is required'); }
      return `${namespace}__${name}`;
  }

  static unformatName(value, namespace) {
      if (value.length+2 < namespace) { return ''; }
  ...
  removeItem(name) {
      window[this._attr.storage].removeItem(WebStorage.formatName(name, this._attr.namespace));
  }
  ...
  getItem(name) {
      try {
          const { value, expires, version } = JSON.parse(window[this._attr.storage].getItem(WebStorage.formatName(name, this._attr.namespace)));
